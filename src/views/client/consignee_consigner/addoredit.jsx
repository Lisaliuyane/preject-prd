...
class AddOrEdit extends ModularParent {
    ...
    show(d) {
        ...
        } else if(d.data && (d.data.shipmentType === 'shipment')) { //仓库出货需求新建
            this.setState({
                openType: 3,
                title:'新建收发货方',
                addressType: d.data.addressType
            })
        } else if (d.data) {
            d.data = Object.assign({}, d.data, {
                openType: 2,
    ...
    //             let areaNameData = res && res.length > 0 ? res.map(d => {
    //                 return d.title
    //             }).join(',') : null
    //             this.setState({areaName: areaNameData})
    //         })
    //     } else {
