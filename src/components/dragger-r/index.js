...
export default class Dragger extends React.Component {
    ...
    move(event) {
        ...
        */
        ...
        if (bounds) {
            ...
            * 如果用户指定一个边界，那么在这里处理
            */
            let NewBounds = typeof bounds === 'string' || typeof bounds === 'function' ? bounds : parseBounds(bounds)
            ...
            } else if (typeof this.props.bounds === 'function') {
                NewBounds = parseBounds(bounds())
            }
        ...
        /**移动时回调，用于外部控制 */
        if (this.props.dragEvent) this.props.dragEvent({...event, deltaX, deltaY}, 'move')
    ...
    onDragStart(event) {
        ...
        if (this.props.dragEvent) this.props.dragEvent(event, 'start')
    ...
    onDragEnd(event) {
        ...
        if (this.props.dragEvent) this.props.dragEvent(event, 'end')
    }
    ...
    componentDidUpdate() {
        let { lastX, lastY, x, y } = this.state
        ...
        */
        ...
        if (bounds) {
            ...
            */
             ...
             * 
             */
            if (isNumber(NewBounds.right) && deltaX > NewBounds.right) {
                isOverBounds = true
                deltaX = Math.min(deltaX, NewBounds.right)
            }
            if (isNumber(NewBounds.bottom) && deltaY > NewBounds.bottom) {
                isOverBounds = true
             ...
             * 
             */
            if (isNumber(NewBounds.left) && deltaX < NewBounds.left) {
                isOverBounds = true
                deltaX = Math.max(deltaX, NewBounds.left)
            }
            if (isNumber(NewBounds.top) && deltaY < NewBounds.top) {
                isOverBounds = true
        ...
        // console.log('isOverBounds', isOverBounds)
        if (isOverBounds) {
            this.setState({
                x: deltaX,
                y: deltaY
