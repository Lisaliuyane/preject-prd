...
class Action extends Component {
    state = {}
    render() { 
        let { onDelete, onEdit, onLook, deltext, record, power, style, isPreventActionEvent } = this.props
...
export const columnsAddNumber = ({columns, title}) => {
    let obj = {
        ...
        render: (text, record, index) => {
            // console.log('columnsAddNumber', record)
            let num = index + 1
            if ('num' in record) {
                num = record.num
            }
...
export const columnsAddAction = (d) => {
    const {columns, that, deltext, title, power, fixed, onDeleteItem, onEditItem, actionView, actionWidth, isPreventActionEvent, isShowActionDel } = d
